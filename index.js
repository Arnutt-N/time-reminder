...
const morningReminder = cron.schedule('25 7 * * *', () => {
  ...
  bot.sendMessage(chatId, getCheckInReminderMessage())
...
const morningMessage = cron.schedule('25 8 * * *', () => {
  ...
  bot.sendMessage(chatId, morningFullMessage)
    .then(() => console.log('Message sent successfully'))
    .catch(err => console.error('Error sending message:', err));
});
...
const eveningReminder = cron.schedule('25 15 * * *', () => {
  console.log('Sending check-out reminder (15:25)... ' + new Date().toISOString());
  bot.sendMessage(chatId, getCheckOutReminderMessage())
    .then(() => console.log('Message sent successfully'))
    .catch(err => console.error('Error sending message:', err));
});
...
const eveningMessage = cron.schedule('25 16 * * *', () => {
  ...
  bot.sendMessage(chatId, eveningFullMessage)
    .then(() => console.log('Message sent successfully'))
    .catch(err => console.error('Error sending message:', err));
});
...
handlers.status = bot.onText(/\/status/, (msg) => {
  bot.sendMessage(msg.chat.id, '✅ บอทกำลังทำงานปกติ และพร้อมส่งข้อความแจ้งเตือนตามเวลาที่กำหนด!')
...
handlers.checkin = bot.onText(/\/checkin/, (msg) => {
  bot.sendMessage(msg.chat.id, getCheckInReminderMessage())
...
handlers.checkout = bot.onText(/\/checkout/, (msg) => {
  bot.sendMessage(msg.chat.id, getCheckOutReminderMessage())
...
handlers.morning = bot.onText(/\/morning/, (msg) => {
  bot.sendMessage(msg.chat.id, getMorningMessage())
...
handlers.evening = bot.onText(/\/evening/, (msg) => {
  bot.sendMessage(msg.chat.id, getEveningMessage())
...
handlers.morningFull = bot.onText(/\/morning_full/, (msg) => {
  // สร้างข้อความครั้งเดียวและเก็บในตัวแปร
  const morningFullMessage = getMorningMessage() + "\n\n" + getCheckInReminderMessage();
  console.log('Sending test morning_full message...');

  // ส่งข้อความเพียงครั้งเดียว
  bot.sendMessage(msg.chat.id, morningFullMessage)
...
handlers.eveningFull = bot.onText(/\/evening_full/, (msg) => {
  ...
  // ส่งข้อความเพียงครั้งเดียว
  bot.sendMessage(msg.chat.id, eveningFullMessage)
    .then(() => console.log('Evening full message sent successfully'))
    .catch(err => console.error('Error sending evening full message:', err));
});

// จัดการข้อความที่ไม่รู้จัก - ใช้ event 'message' แบบมีเงื่อนไขพิเศษเพื่อป้องกันการตอบซ้ำซ้อน
bot.on('message', (msg) => {
  const chatId = msg.chat.id;
  const messageText = msg.text || '';

  // ตรวจสอบว่าไม่ใช่คำสั่งที่รู้จักและยังไม่ได้ถูกจัดการโดย handlers ข้างต้น
  if (messageText && !messageText.startsWith('/')) {
    bot.sendMessage(chatId, `ขอบคุณสำหรับข้อความของคุณ! หากต้องการดูคำสั่งที่มี พิมพ์ /start`)
